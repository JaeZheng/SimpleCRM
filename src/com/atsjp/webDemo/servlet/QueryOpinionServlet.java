...
@WebServlet("/QueryOpinionServlet")
public class QueryOpinionServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private List<Opinion> tempCu = new LinkedList<Opinion>();
	private UserService us = new UserService();
    private Page pageService = new Page();

	public QueryOpinionServlet() {
		super();
	}
...
//        }
        if(index!=null && !index.equals("")){
            ...
            for(int i=0; cookies!=null && i<cookies.length; i++){
                if(cookies[i].getName().equals("cindex")){
...
//                    System.out.println("重设cookie值");
            ...
            if(noCookieFlag){
                Cookie cookie = new Cookie("cindex", URLEncoder.encode(index, "UTF-8"));
...
//                System.out.println("index值:" + index);
        ...
        // 获取查询总个数
        pageService.queryOpinionCount(index);
        // 维护当前页
...
//        System.out.println("当前页：" + Integer.toString(currentPage));
		// 进行查找company信息
		tempCu = us.queryOpinionList(index, currentPage, pageSize);
		...
		if (!tempCu.isEmpty()) {
			request.setAttribute("OpinionList", tempCu);
			request.setAttribute("result", index);
			request.getRequestDispatcher("./manager/queryOpinion.jsp")
					.forward(request, response);
		} else {
			request.setAttribute("OpinionList", null);
            request.setAttribute("result", index);
			request.getRequestDispatcher("./manager/queryOpinion.jsp")
					.forward(request, response);
		}
