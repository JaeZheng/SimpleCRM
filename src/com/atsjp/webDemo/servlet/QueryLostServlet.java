 ...
 * 处理分页查询获取lost队列
 */
@WebServlet("/QueryLostServlet")
public class QueryLostServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private List<Lost> tempCu = new LinkedList<Lost>();
	private UserService us = new UserService();
    private Page pageService = new Page();

	public QueryLostServlet() {
		super();
	}
...
//                System.out.println("index值:" + index);
        ...
        // 获取查询总个数
        pageService.queryLostCount(index);
        // 维护当前页
...
//        System.out.println("当前页：" + Integer.toString(currentPage));
		// 进行查找company信息
		tempCu = us.queryLostList(index, currentPage, pageSize);
		...
		if (!tempCu.isEmpty()) {
			request.setAttribute("LostList", tempCu);
			request.setAttribute("result", index);
			request.getRequestDispatcher("./manager/queryLost.jsp")
					.forward(request, response);
		} else {
			request.setAttribute("LostList", null);
            request.setAttribute("result", index);
			request.getRequestDispatcher("./manager/queryLost.jsp")
					.forward(request, response);
		}
